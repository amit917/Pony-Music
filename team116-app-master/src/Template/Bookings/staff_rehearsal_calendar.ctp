...
$day=$pieces[2];
if($pieces[2][0]==0){
$fp = explode("0", $pieces[2]);
$day=$fp[1];


}
echo "
<th><a href='../../users/admin_dashboard?year=$fullYear&month=$pieces[1]&day=$day'>".$pieces[2]."/".$pieces[1]."</a>
</th>
";
if($tempToday<$daysinM){
$tempToday++;
...
$colour="><a class='nav-link'
             href='../../Bookings/staff_rehearsal_form?date=$arDate&timestamp=$arAMTime$arRoomS1&price=$room'>$$fee</a>";
$count=1;
for($i=0;$i
<sizeof
($date);$i++) {
...
<sizeof
($date);$i++) {
$stmt = $conn->execute("select room_usages.room_id,room_usages_date,room_usages.id,booking_session from room_usages
inner join bookings b on room_usages.booking_id = b.id

where room_usages.room_id=" .$roomId." AND room_usages_session='AM' and booking_session='AM' ");
...
$details=$conn->execute("select b.id,display_name,booking_total_charge from room_usages
inner join bookings b on room_usages.room_id = b.room_id
inner join bookings_clients bc on b.id = bc.booking_id
inner join clients c on bc.client_id = c.id where room_usages.id=$id[2] and room_usages_date=booking_date_from and
room_usages.booking_id=b.id and booking_session='AM' and b.status='completed'" );
foreach ($details->fetchAll()as$detail):
    $fee=explode('.',$detail[2])[0];


$colour = "bgcolor='#66ffcc'><a class='nav-link'
                                href='../../Bookings/rehearsal_booking_detail?BookingId=$detail[0]'>$detail[1]<br/>$$fee ";
...
$stmt = $conn->execute("select room_usages.room_id,room_usages_date,room_usages.id,booking_session from room_usages
inner join bookings b on room_usages.booking_id = b.id
where room_usages.room_id=" .$roomId." AND room_usages_session='PM' and booking_session='PM'");
...
inner join bookings_clients bc on b.id = bc.booking_id
inner join clients c on bc.client_id = c.id where room_usages.id=$id[2] and room_usages_date=booking_date_from and
room_usages.booking_id=b.id and b.booking_session='PM' and b.status='completed' " );
foreach ($details->fetchAll()as$detail):
...
$colour = "bgcolor='#66ffcc'><a class='nav-link'
                                href='../../Bookings/rehearsal_booking_detail?BookingId=$detail[0]'>$detail[1]<br/>$$fee<br>";
    // $colour = "onclick=' myFunction($detail[0])' bgcolor='#66ffcc'>$detail[1]<br/>$ $detail[2]<br> Assets<br>";
...
<table class="table table-bordered table-responsive-sm" id="calendar">
    ...
    <!--        <form method="post" action="iteration2_test_calendar.ctp?key=">-->
        ...
        <?php
                ...
                echo $this->Html->link('previous week'

            , "/bookings/staff_rehearsal_calendar?key=$pWeek&year=$ptoYear", array('class' => 'btn btn-primary
            btn-round btn-sm'));
        ...
        <th colspan="2">
<?php echo $this->Html->link('today', "/bookings/staff_rehearsal_calendar", array('class' => 'btn btn-default
        btn-round btn-sm'));?>
        </th>
        <th colspan="2">    <?php
                $nWeek=$week+1;
                if($nWeek>$numberOfWeeks){
            $nWeek=1;
            $ntoYear=$toYear;
            $ntoYear+=1;
            ...
            echo $this->Html->link('next week', "/bookings/staff_rehearsal_calendar?key=$nWeek&year=$ntoYear",
...
<th>
    ...
    <?php
            $stmt=$conn->execute('select t.id,room_number,session_night_charge from sessions inner join rooms t on
    sessions.id = t.session_id where t.session_id=1');
    ?>
    <?php foreach($rows=$stmt->fetchAll() as $i=>$room): ?>
    <tr>
        <th>Room<?= $room[1] ?></th>
        <td
        <?= checkUsagePM($arDate,$arRoomId1[$i],'1',$arDate[0],$arPMTime[0],$arRoomS1[$i],$room[2]) ;?>
        </td>
        <td
        <?= checkUsagePM($arDate,$arRoomId2[$i],'2',$arDate[1],$arPMTime[1],$arRoomS2[$i],$arPMCharge[1]) ;?>
        </td>
        <td
        <?= checkUsagePM($arDate,$arRoomId3[$i],'3',$arDate[2],$arPMTime[2],$arRoomS3[$i],$arPMCharge[2]) ;?>
        </td>
        <td
        <?= checkUsagePM($arDate,$arRoomId4[$i],'4',$arDate[3],$arPMTime[3],$arRoomS4[$i],$arPMCharge[3]) ;?>
        </td>
        <td
        <?= checkUsagePM($arDate,$arRoomId5[$i],'5',$arDate[4],$arPMTime[4],$arRoomS5[$i],$arPMCharge[4]) ;?>
        </td>
        <td
        <?= checkUsagePM($arDate,$arRoomId6[$i],'6',$arDate[5],$arPMTime[5],$arRoomS6[$i],$arPMCharge[5]) ;?>
        </td>
        <td
        <?= checkUsagePM($arDate,$arRoomId7[$i],'7',$arDate[6],$arPMTime[6],$arRoomS7[$i],$arPMCharge[6]) ;?>
        </td>
    </tr>
    <?php endforeach; ?>
    ...
    <?php
            $stmt=$conn->execute('select t.id,room_number,session_day_charge,session_id from sessions inner join rooms t
    on sessions.id = t.session_id where t.session_id=1');
    ?>
    <?php foreach($rows=$stmt->fetchAll() as $i=>$room): ?>
    <tr>
        <th>Room<?= $room[1] ?></th>
        <td
        <?= checkUsage($toYear,$arDate,$arRoomId1[$i],'1','AM',$arDate[0],$arAMTime[0],$arRoomS1[$i],$room[2],$i) ;?>
        </td>
        <td
        <?= checkUsage($toYear,$arDate,$arRoomId2[$i],'2','AM',$arDate[1],$arAMTime[1],$arRoomS2[$i],$arAMCharge[1],$i) ;?>
        </td>
        <td
        <?= checkUsage($toYear,$arDate,$arRoomId3[$i],'3','AM',$arDate[2],$arAMTime[2],$arRoomS3[$i],$arAMCharge[2],$i) ;?>
        </td>
        <td
        <?= checkUsage($toYear,$arDate,$arRoomId4[$i],'4','AM',$arDate[3],$arAMTime[3],$arRoomS4[$i],$arAMCharge[3],$i) ;?>
        </td>
        <td
        <?= checkUsage($toYear,$arDate,$arRoomId5[$i],'5','AM',$arDate[4],$arAMTime[4],$arRoomS5[$i],$arAMCharge[4],$i) ;?>
        </td>
        <td
        <?= checkUsage($toYear,$arDate,$arRoomId6[$i],'6','AM',$arDate[5],$arAMTime[5],$arRoomS6[$i],$arAMCharge[5],$i) ;?>
        </td>
        <td
        <?= checkUsage($toYear,$arDate,$arRoomId7[$i],'7','AM',$arDate[6],$arAMTime[6],$arRoomS7[$i],$arAMCharge[6],$i) ;?>
        </td>
    </tr>
    <?php endforeach;?>
