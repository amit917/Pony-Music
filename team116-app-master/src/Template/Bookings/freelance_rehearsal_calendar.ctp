...
href='../../bookings/public_rehearsal_form?hash=hbeiebifvieiefei32782dgdqdksqjbdhevdhvefhveqjhfjehfvegyi23ri2&date=$arDate&timestamp=$arAMTime$arRoomS1&price=$room'>$ $fee </a>
";
 $count=1;
...
where room_usages.room_id=$roomId AND room_usages_session='AM' AND b.status='completed'");
        ...
        foreach ($reval = $stmt->fetchAll() as $id) {
            if ($id[1]==$date[$i]) {
                ...
                if($str==$sessionId){
                    $sql=$conn->execute("select * from sessions where id=$str");
...
<table class="table table-bordered table-responsive-sm" id="calendar">
    ...
    <tr class="noBorder">
        ...
        <th colspan="2">
                    ...
                    echo $this->Html->link('Previous week', ['controller'=>'bookings','action'=>"freelance_rehearsal_calendar?key=$pWeek&year=$ptoYear"], array('class' => 'btn btn-primary btn-round btn-sm'));
   ...
   <th colspan="2">                    
       <?php echo $this->Html->link('today', "/bookings/freelance_rehearsal_calendar", array('class' => 'btn btn-default btn-round btn-sm'));?>
</th>
        <th colspan="2">               <?php
                    $nWeek=$week+1;
                    ...
                    echo $this->Html->link('Next week', ['controller'=>'bookings','action'=>"freelance_rehearsal_calendar?key=$nWeek&year=$ntoYear"], array('class' => 'btn btn-primary btn-round btn-sm'));
        ...
        <table class="table table-bordered table-responsive-sm" id="calendar">
            ...
            $stmt=$conn->execute(' select * from sessions');
            ?>
            <?php foreach($rows=$stmt->fetchAll() as $time): $a=explode(':',$time[1]);
            $q=explode(':',$time[2]);
