<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="category form-category">* Required fields</div>
         <hr class="mb-4">
         <div>
             <?= $this->Html->link('Back to Staff', ['controller' => 'staffs', 'action' => 'index'], ['class' => 'btn btn-default float-left']) ?>
        <?= $this->Form->button(__('Submit'), ['class' => 'btn btn-primary float-right']) ?>
        <?= $this->Form->end() ?>
        <?= $this->Form->postLink(__('Delete'), ['controller' => 'users','action' => 'delete',
                                $admin->id],
                                ['class'=>' btn btn-danger
                                float-left', 'confirm' => __('Are you sure you want to delete'.' '.$admin->fname.' '.$admin->lname.'?', $admin->id)]) ?> 
         </div>
    </div>
